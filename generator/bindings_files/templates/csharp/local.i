...
*/
%typemap("csout", excode=SWIGEXCODE) SBMLTask*
{
  SBMLTask ret = (SBMLTask) libsbml.DowncastSBase($imcall, $owner);$excode
  return ret;
}
...
*/
%typemap("csout", excode=SWIGEXCODE) SBMLRange*
{
  SBMLRange ret = (SBMLRange) libsbml.DowncastSBase($imcall, $owner);$excode
  return ret;
}
...
* Convert Rule objects into the most specific object possible.
*/
%typemap("csout", excode=SWIGEXCODE) SBMLSimulation*
{
  SBMLSimulation ret = (SBMLSimulation) libsbml.DowncastSBase($imcall, $owner);$excode
  return ret;
}
...
*/
%typemap("csout", excode=SWIGEXCODE) SBMLOutput*
{
  SBMLOutput ret
  = (SBMLOutput) libsbml.DowncastSBase($imcall, $owner);$excode
  return ret;
}
...
* Convert SimpleSpeciesReference objects into the most specific object possible.
*/
%typemap("csout", excode=SWIGEXCODE) SBMLChange*
{
  SBMLChange ret
  = (SBMLChange) libsbml.DowncastSBase($imcall, $owner);$excode
  return ret;
}
...
* for C# proxy classes of SBase subclasses and classes in libSEDML.
*
...
* SBMLConstructorException (C# class) which is derived from
...
*  - writeMathMLToString()
*
* Which take either an SBMLConstructorException or MathMLDocument as input.
*/
%typemap(csin) SBMLConstructorException   * "SBMLConstructorException.getCPtr($csinput)";
...
//  
//  1) SBMLReader::readSEDML(const std::string& filename)
//  2) readSEDML(const char* filename)
//  3) SBMLWriter::writeSEDML(SBMLConstructorException*, const std::string& filename)
//  4) writeSEDML(SBMLConstructorException*, const char* filename)
//
