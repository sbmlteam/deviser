...
uses the \GroupsPackage. We use the same conventions as are used in the
...
rule, it does not conform to the \GroupsPackage specification. (Mnemonic
...
\GroupsPackage specification is concerned; however, it indicates that
...
the \GroupsPackage package.

For \notice convenience and brevity, we use the shorthand
``\token{groups:\-x}'' to stand for an attribute or element name
\token{x} in the namespace for the \GroupsPackage package, using the
namespace prefix \token{groups}. In reality, the prefix string may be
different from the literal ``\token{groups}'' used here (and indeed, it
can be any valid XML namespace prefix that the modeler or software
chooses). We use ``\token{groups:\-x}'' because it is shorter than to
write a full explanation everywhere we refer to an attribute or element
in the \GroupsPackage namespace.
...
Level~3 Specification for Groups, Version~1 \sec{xml-namespace}.)}
...
SBML Level~3 Specification for Groups, Version~1 \sec{xml-namespace}.)}

\subsubsection*{General rules about identifiers}

\validRule{groups-10301}{(Extends validation rule \#10301 in the
...
\val{false}. (Reference: SBML Level~3 Specification for Groups,
...
SBML Level~3 Specification for Groups, Version~1,
...
object may only contain \Group objects. (Reference: SBML Level~3
Specification for Groups, Version~1, \sec{extended-model-class}.)}
...
a \ListOfGroups object. (Reference: SBML Level~3 Specification for
Groups, Version~1, \sec{extended-model-class}.)}
...
SBML Level~3 Specification for Groups, Version~1, \sec{group-class}.)}
...
be one of the following: \val{classification}, \val{partonomy} or
\val{collection}. (Reference: SBML Level~3 Specification for Groups,
Version~1, \sec{group-class}.)}
...
Specification for Groups, Version~1, \sec{group-class}.)}
...
Specification for Groups, Version~1, \sec{listofmembers-class}.)}

\validRule{groups-20309}{A \ListOfMembers object may have the optional
SBML Level~3 Core attributes \token{metaid} and \token{sboTerm}. No
other attributes from the SBML Level~3 Core namespaces are permitted on
a \ListOfMembers object. (Reference: SBML Level~3 Specification for
Groups, Version~1, \sec{listofmembers-class}.)}
...
\ListOfMembers object. (Reference: SBML Level~3 Specification for
Groups, Version~1, \sec{listofmembers-class}.)}
...
attributes \token{groups:\-id}, \token{groups:\-name},
\token{groups:\-idRef} and \token{groups:\-metaIdRef}. No other
attributes from the SBML Level~3 Groups namespaces are permitted on a
\Member object. (Reference: SBML Level~3 Specification for Groups,
Version~1, \sec{member-class}.)}
...
Level~3 Specification for Groups, Version~1, \sec{member-class}.)}

\validRule{groups-20405}{The value of the attribute
\token{groups:\-idRef} of a \Member object must be the identifier of an
existing object derived from the \SBase class and defined in the
...
\token{groups:\-metaIdRef} of a \Member object must be the 'metaid' of
