                                 ...
                                 const char* value) const
{
  ...
  {
    value = getBindingStatusAsString().c_str();
    return_value = LIBSBML_OPERATION_SUCCESS;
  }
  else if (attributeName == "component")
  {
    value = getComponent().c_str();
    return_value = LIBSBML_OPERATION_SUCCESS;
  }
                                     ...
                                     expectedAttributes)
{
  ...
  if (static_cast<ListOfOutwardBindingSites*>(getParentSBMLObject())->size() <
    2)
  {
  ...
  for (int n = numErrs-1; n >= 0; n--)
  {
    ...
    else if (log->getError(n)->getErrorId() == UnknownCoreAttribute)
    {
      ...
      log->remove(UnknownCoreAttribute);
      log->logPackageError("multi",
        MultiOutwardBindingSiteAllowedCoreAttributes, pkgVersion, level, version,
          details);
    }
  }
